...
// Foundations Drawer — a slide-in panel built on top of Modal (copied from
// foundations.significa.co/ui/drawer, with its two imports repointed to this project's paths:
// `@/components/ui/modal` and `@/lib/utils/classnames`). It's a thin styling wrapper: Modal
// supplies the <dialog> top-layer, focus trap, and open/close plumbing; this file only adds
// the edge-anchored slide animation and the Header/Actions layout regions.
...
// All the geometry (detach distance, corner radius, slide direction per side) lives here.
// The `--drawer-*` custom properties are consumed by Header/Actions below for their bleed and
// sticky padding, so the whole panel shares one spacing scale.
...
// Cancels out the content's own horizontal padding so a child (header/actions bar) can span
// the full drawer width edge to edge.
const DrawerBleed = ({
  className,
  ...props
