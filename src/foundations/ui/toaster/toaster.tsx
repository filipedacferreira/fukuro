...
const createToastStore = (): ToastStore => {
  ...
  const timers = new Map<
    string,
    { startTime: number; remainingTime: number; timeoutId: ReturnType<typeof setTimeout> | null }
  >();

  const getToastId = () => Date.now().toString() + Math.random().toString(36).slice(2, 9);
  ...
  return {
    ...
    add(toastData) {
      ...
      timers.set(id, { startTime: Date.now(), remainingTime: duration, timeoutId: timeout });
    },
    remove(id) {
...
const useToastStore = () =>
  useSyncExternalStore(toastStore.subscribe, () => toastStore.toasts, () => toastStore.toasts);

const toast = (t: Omit<Toast, 'id'>) => toastStore.add(t);
    ...
    >
      <AnimatePresence>
        {toasts.map((t, index) => (
          ...
          >
            <ToasterItem toast={t} onDismiss={() => toastStore.remove(t.id)} />
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
